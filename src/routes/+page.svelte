<script lang="ts">
	...
	async function loadLocalFiles(files: FileList, index = 0) {
		if (!audioContext) return;
	...
	async function loadPlaylist() {
		try {
			...
			if (playlist.length > 0) {
				// Start at a random track
				currentTrackIndex = Math.floor(Math.random() * playlist.length);
				// Preload the random starting track
				await preloadFirstTrack();

				// If user already clicked, start playing
				if (userClickedStart) {
					await startPlayback();
				}
			}
	...
	async function handleStartPlaylist() {
		...
		// If music is already loaded, start playing
		if (preloadedBuffer) {
			await startPlayback();
		} else {
			// Otherwise, wait for it to load (loadPlaylist will start it)
			showStartButton = false;
		}
	}
	...
	onMount(async () => {
		...
		// Start visualizer rendering immediately
		rendering = true;
		startRenderer();

		// Load playlist (will show start button when ready)
		await loadPlaylist();
	...
	<div class="bottomToolbar" class:visible={showToolbars}>
		<div id="audioSelectWrapper">
			<button id="localFileBut" onclick={handleFileSelect}>
				<img src="/files.svg" alt="Load local files" width="36" height="36" />
			</button>
			<button id="micSelect" onclick={handleMicSelect}>
				<img src="/microphone.svg" alt="Use Mic" width="36" height="36" />
			</button>
		</div>
	</div>
...
<style>
	...
	#startOverlay:hover .clickPrompt {
		opacity: 1;
	}
